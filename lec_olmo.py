"""
LEC Pretraining — OLMo-2 + PLE + Router via HuggingFace Trainer
================================================================
Trains OLMo-2 with Lightweight Expert Conditioning from scratch.
All settings identical to pretrain_baseline.py except LEC additions.
...
CONFIG = {
    ...
    # Misc
    "seed": 42,
    "output_dir": "./checkpoints_lec",
    "use_wandb": False,
    "wandb_project": "lec-pretrain",
        ...
        with torch.no_grad():
            for l in range(self.num_layers):
                if self.num_experts <= self.ple_dim:
                    ...
                    self.embeddings[:, l, :] = q[: self.num_experts] * 0.02
                else:
                    nn.init.normal_(self.embeddings[:, l, :], std=0.01)
...
class OLMo2LEC(Olmo2ForCausalLM):
    """
    OLMo-2 with PLE conditioning injected via forward hooks.
    ...
    def __init__(self, config, lec_config: dict):
        ...
        self.ple = PLETable(K, L, lec_config["ple_dim"], H)
...
def build_model(config: dict) -> OLMo2LEC:
    ...
    lec_config = {
        k: config[k]
        for k in ["num_experts", "top_k", "ple_dim", "lb_ema_alpha", "lb_bias_lr"]
    }
    model = OLMo2LEC._from_config(
        olmo_cfg,
        lec_config=lec_config,
        attn_implementation=config["attn_implementation"],
    )
    n = sum(p.numel() for p in model.parameters())
    print(f"[Model] OLMo-2 + LEC: {n:,} params ({n / 1e9:.2f}B)")
    return model


# ── Tokenizer + Dataset (identical to baseline) ───────────────────────────────
...
class LECTrainer(Trainer):
    ...
    def _flush_metrics(self, split: str, prefix: str = "") -> dict:
        ...
        averaged = {
            f"{prefix}{key}": value / count for key, value in self._metric_sums[split].items()
        }
...
def train(config: dict):
    ...
    print(f"[Train] Total: {tokens_per_step * config['max_steps'] / 1e9:.1f}B tokens\n")
    ...
    trainer.train()
    model.remove_hooks()
    print("[Train] LEC pretraining complete.")
